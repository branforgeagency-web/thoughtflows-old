...
// Import all images from Home/Companies.js location
...
const PlacementsCompanies = () => {
    return (
        <div className="placements-companies-marquee">
            {/* First Row - Left */}
            <div className="marquee-row">
                <Marquee gradient={true} speed={50} direction="left" gradientColor={[249, 249, 249]}>
                    {row1.map((image, index) => (
                        <div key={`row1-${index}`} className="company-logo-wrapper">
                            <img
                                src={image}
            ...
            {/* Second Row - Right (Opposite) */}
            <div className="marquee-row">
                <Marquee gradient={true} speed={50} direction="right" gradientColor={[249, 249, 249]}>
                    {row2.map((image, index) => (
                        <div key={`row2-${index}`} className="company-logo-wrapper">
                            <img
                                src={image}
                                alt={`Company Logo ${index + 1}`}
                                className="company-logo-img"
            ...
            {/* Third Row - Left (Same as First) */}
            <div className="marquee-row">
                <Marquee gradient={true} speed={50} direction="left" gradientColor={[249, 249, 249]}>
                    {row3.map((image, index) => (
                        <div key={`row3-${index}`} className="company-logo-wrapper">
                            <img
                                src={image}
                                alt={`Company Logo ${index + 1}`}
                                className="company-logo-img"
